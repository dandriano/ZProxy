...
public class ProxyCurveTests
{
    ...
    {
        var serverCert = Cert.GenerateCert();
        var serverPubKey = serverCert.PublicKey;
        var clientCert = Cert.GenerateCert();
        ...
        var client = new Client(serverEndpoint, serverPubKey, clientCert,
                                socksHandler, _loggerFactory.CreateLogger<Client>());

        // run everything inside NetMQRuntime
        ...
        await WaitForPortAsync(socksPort, TimeSpan.FromSeconds(5));

        try
        {
            // try socks5 (default right now)
            ...
            await socksStream.WriteAsync(requestBytes);
...
#region [Helpers]
    ...
    private static async Task WaitForPortAsync(int port, TimeSpan timeout)
    {
        ...
        } while (DateTime.Now.Subtract(s) < timeout);

        Assert.Fail($"SOCKS5 port {port} did not become available");
    }
    ...
    private static async Task HandleHttpConnection(TcpClient client, string response, CancellationToken ct)
    {
        using (client)
        {
            ...
            {
                int read = await stream.ReadAsync(buf.AsMemory(total, 1), ct);
                if (read == 0) break;
                total++;
                if (total >= 4 &&
    ...
    {
        // greeting
        await stream.WriteAsync((new byte[] { 0x05, 0x01, 0x00 }).AsMemory(0, 3));
        await stream.FlushAsync();
        var resp = new byte[2];
        if (await stream.ReadAsync(resp.AsMemory(0, 2)) < 2) 
            return false;
        ...
        await stream.WriteAsync(ms.ToArray().AsMemory(0, (int)ms.Length));
        await stream.FlushAsync();

        var reply = new byte[10];
        if (await stream.ReadAsync(reply.AsMemory(0, 10)) < 10) 
            return false;
